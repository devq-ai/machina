...
app = FastAPI(
    title="redis-mcp",
    description="Production MCP Server for redis-mcp",
    version="1.0.0"
)
...
async def list_tools():
    ...
    tools = [
        {
            "name": "set_key",
            "description": "Set Key for redis-mcp",
            "parameters": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "get_key",
            "description": "Get Key for redis-mcp",
            "parameters": {
                "type": "object",
                "properties": {
                    "input": {"type": "string", "description": "Input parameter"}
        ...
        {
            "name": "list_keys",
            "description": "List Keys for redis-mcp",
            "parameters": {
                "type": "object",
                "properties": {
                    "input": {"type": "string", "description": "Input parameter"}
...
async def call_tool(request: dict):
    ...
    if "status" in tool_name:
        ...
        result["result"] = {
            "name": "redis-mcp",
            "type": "mcp_server",
            "capabilities": ['set_key', 'get_key', 'list_keys'],
            "version": "1.0.0"
        }
...
if __name__ == "__main__":
    print(f"🚀 Starting {'redis-mcp'} on port 8033")
    uvicorn.run(
        "__main__:app",
        host="0.0.0.0",
        port=8033,
